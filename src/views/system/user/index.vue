...
//上传的url
const uploadUrl = "/dev-api/api/file/upload"
...
<!--            />-->
			  ...
			  <el-avatar v-if="scope.row.avatarUrl&&scope.row.avatarUrl.length>0" shape="square" :size="40" :src="scope.row.avatarUrl" />
							<el-avatar shape="square" :size="40" v-else>{{scope.row.name.length>2?scope.row.name.substring(0,2):scope.row.name}}</el-avatar>
				  </template>
			  </el-table-column>
            <el-table-column
              key="phone"
