	...
	userFormRef.value.validate((valid: any) => {
		if (valid) {
			const userId = formData.id;
			 formData.fieldData=proxy.$deepCopy(formData)
			loading.value = true;
			if (userId) {
	...
	<div class="app-container">
		...
		>
			...
			>
				...
				<!--扩展字段 userFieldDataList-->

				<template v-for="item in formData.userFieldDataList">

					<el-form-item :label="item.name" :prop="item.key">

						<el-select v-if="item.type=='SingleSelect'" v-model="formData[item.key]"
						>
							<el-option
									v-for="item in JSON.parse(item.props).options"
									:key="item.key"
									:label="item.value"
									:value="item.key"
							/>
						...
						<el-select multiple v-model="formData[item.key]"
											 collapse-tags
						...
						<el-date-picker value-format="YYYY-MM-DD"
								v-if="item.type=='Date'" v-model="formData[item.key]"
								type="date"
						/>
						<el-time-picker value-format="HH:mm:ss"
								v-if="item.type=='Time'" v-model="formData[item.key]"
								arrow-control
						/>
						<el-date-picker value-format="YYYY-MM-DD HH:mm:ss"
								v-if="item.type=='DateTime'" v-model="formData[item.key]"
								type="datetime"
						/>
						<el-input v-if="item.type=='Input'" v-model="formData[item.key]"></el-input>
						<el-input
								type="textarea" v-model="formData[item.key]"
								v-if="item.type=='Textarea'"></el-input>
						<el-input-number v-model="formData[item.key]" v-if="item.type=='Number'"></el-input-number>
					</el-form-item>
				</template>
