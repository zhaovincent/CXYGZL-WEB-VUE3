  ...
  <div>
    <div class="titlebar">
      ...
      <div class="f2">
        ...
        <span class="center_t" effect="dark" :activeStep="activeStep==3" @click="activeStep=3">
							<span :activeStep="activeStep==3">4</span>
							<span>扩展设置</span>
					</span>
    ...
    <step4 v-show="activeStep === 3" ref="step4Ref"/>
  ...
  if (util.isNotBlank(flowId)) {
    ...
    getFlowDetail(flowId).then(res => {
      ...
      if (data.settings) {
        ...
        if (setting.dbRecord) {
          setting.dbRecord.oldEnable = setting.dbRecord.enable;
        }
        store.setStep4(setting)
...
const submitFlow = () => {
  step3Ref.value.getProcessData().then(res => {
    ...
    flow.settings = JSON.stringify(step4);
